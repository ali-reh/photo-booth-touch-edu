    ...
    <main className="kiosk-container bg-[#102a2b] text-[#f8f4e8]">
      ...
      <div className="relative mx-auto flex h-full w-full max-w-2xl flex-col items-center px-5 py-7 sm:px-10 sm:py-10">
        ...
        <div className="flex w-full flex-col items-center gap-3 pt-6">
          ...
          {hasCapturedPhoto ? <p className="text-center text-base font-semibold text-[#f8e2a5]">Photo uploaded. Preparing visitor details...</p> : <Button type="button" size="lg" disabled={!isReady || countdown !== null || isUploading} onClick={beginCapture} className="min-w-56 border-2 border-[#fff1c5] bg-[#efc36f] text-[#173638] hover:bg-[#f7d48e]">{isUploading ? 'Uploading photo...' : isReady ? 'Capture photo' : 'Starting camera...'}</Button>}
